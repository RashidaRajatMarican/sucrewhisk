const products=[
{id:1,name:"Taro Gelato",category:"Gelato",price:7.5,emoji:"🍠",image:"images/taro.png",desc:"Creamy taro gelato with a smooth, delicately sweet flavour."},
...
function productVisual(p,cart=false){if(p.image){return `<img class="${cart?'cart-product-image':'product-image'}" src="${p.image}" alt="${p.name}">`}return p.emoji}
function renderProducts(){const visible=products.filter(p=>p.category===state.category);byId("productGrid").innerHTML=visible.map(p=>`<article class="product-card"><div class="product-art">${productVisual(p)}</div><h4>${p.name}</h4><p>${p.desc}</p><div class="product-footer"><span class="price">${money(p.price)}</span><button class="add-button" onclick="addToCart(${p.id})">+ Add</button></div></article>`).join("")}
...
function renderCart(){const entries=Object.entries(state.cart).filter(([,q])=>q>0);if(!entries.length){byId("cartItems").innerHTML='<div class="empty">Your cart is empty. Add a little sweetness first ♡</div>'}else{byId("cartItems").innerHTML=entries.map(([id,q])=>{const p=products.find(x=>x.id===Number(id));return `<div class="cart-item"><div class="cart-thumb">${productVisual(p,true)}</div><div><strong>${p.name}</strong><div class="price">${money(p.price)}</div><div class="qty"><button onclick="changeQty(${p.id},-1)">−</button><span>${q}</span><button onclick="changeQty(${p.id},1)">+</button><button class="remove-button" onclick="removeItem(${p.id})">Remove</button></div></div><strong>${money(p.price*q)}</strong></div>`}).join("")}renderTotals()}
